...
export function SaleWindowFields({
  saleWindows,
  stages,
  inventoryCategories,
  rateTypes,
...
function SaleWindowCard({
  ...
  const groups = groupOffersByTarget(saleWindow.offers, stages);
                ...
                >
                  <span className="text-sm font-semibold">
                    {inventoryCategories.find(
                      (inventoryCategory) => inventoryCategory.key === offer.inventoryCategoryKey,
                    )?.name ?? "(席種未設定)"}
                  </span>
...
function groupOffersByTarget(offers: DraftOffer[], stages: DraftStage[]) {
  const groups = new Map<string, DraftOffer[]>();

  for (const offer of offers) {
    const key = [...offer.stageKeys].sort().join(",");
  ...
  return Array.from(groups.entries()).map(([key, groupOffers]) => {
    const names = groupOffers[0]?.stageKeys
      .map((stageKey) => stages.find((stage) => stage.key === stageKey)?.name)
      .filter((name): name is string => Boolean(name));
    const isPass = (groupOffers[0]?.stageKeys.length ?? 0) > 1;
