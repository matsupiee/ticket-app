...
test("主催者が登録後にウィザードで整理番号先着イベントを作成して公開できる", async ({ page }) => {
  ...
  await page.getByRole("button", { name: "迷ったら詳細イベント作成ではじめる" }).click();
...
test("簡単作成で1公演のイベントを作成できる", async ({ page }) => {
  const authState = {
    user: { id: "organizer-user", name: "主催者ユーザー", email: "simple-e2e@example.com" },
  };
  const organizer = {
    eventOrganizerId: "organizer-e2e",
    name: "渋谷イベント制作",
  ...
  await page.goto("/events/new");
  await expect(page.getByRole("heading", { name: "イベントを作成" })).toBeVisible();
  await page.getByRole("button", { name: /簡単イベント作成/ }).click();

  await page.getByLabel("イベント名").fill("E2E簡単公演");
  await page.getByLabel("説明").fill("簡単作成で登録する先着販売イベントです。");
  await page.getByRole("button", { name: "次へ" }).click();

  await expect(page.getByRole("heading", { name: "公演・券種" })).toBeVisible();
  await page.getByLabel("公演の名称").fill("本公演");
  await page.getByLabel("公演の会場").fill("渋谷ホール");
  await page.getByLabel("公演の月日").fill("2026-09-01");
  await page.getByLabel("公演の開場時刻").fill("17:30");
  await page.getByLabel("公演の開始時刻").fill("18:30");
  await page.getByLabel("席種1の名称").fill("スタンディング");
  await page.getByLabel("席種1の在庫数").fill("120");
  await page.getByLabel("席種1の価格").fill("4500");
  await page.getByRole("button", { name: "次へ" }).click();

  await expect(page.getByRole("heading", { name: "販売受付" })).toBeVisible();
  await page.getByRole("button", { name: "＋ 受付を追加（一般販売など）" }).click();
  await page.getByLabel("受付名").fill("一般販売");
  await page.getByLabel("申込開始").fill("2026-08-01T10:00");
  await page.getByLabel("申込終了").fill("2026-08-31T23:59");
  await page.getByRole("button", { name: "＋ 券を追加" }).click();
  await expect(page.getByRole("button", { name: "通し券（複数公演）" })).toHaveCount(0);
  await page.getByRole("button", { name: "追加する" }).click();
  await page.getByRole("button", { name: "作成して公開" }).click();

  await expect(page.getByText("イベントを作成して公開しました")).toBeVisible();
  await expect(page.getByRole("heading", { name: "E2E簡単公演" })).toBeVisible();

  const event = store.events[0];
  expect(event).toMatchObject({
    name: "E2E簡単公演",
    performances: [{ name: "本公演", venueName: "渋谷ホール" }],
    seatCategories: [{ name: "スタンディング" }],
    rateTypes: [{ name: "一般" }],
  });
  expect(event.inventoryPools[0]).toMatchObject({ capacity: 120 });
  expect(event.saleWindows[0]?.offers[0]?.rates[0]).toMatchObject({ price: 4_500 });
  expect(event.saleWindows[0]?.offers[0]?.entitlements).toHaveLength(1);
});
