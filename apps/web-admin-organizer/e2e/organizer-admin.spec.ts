...
test("主催者が登録後に整理番号先着イベントを作成して公開できる", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: "整理番号・先着イベントを作成" })).toBeVisible();
  ...
  await expect(page.getByRole("link", { name: "E2E整理番号公演" })).toBeVisible();
  await expect(page.getByText("販売中", { exact: true })).toBeVisible();
  await expect(page.getByText("整理番号", { exact: true })).toBeVisible();
  await expect(page.getByText("先着", { exact: true }).first()).toBeVisible();

  await page.getByRole("link", { name: "E2E整理番号公演" }).click();
  ...
  await expect(page.getByText("残り 30枚")).toBeVisible();
...
async function mockRpc(
  ...
  await page.route("**/rpc/**", async (route) => {
    ...
    if (pathname.endsWith("/organizer/event/create")) {
      events.splice(0, events.length, buildCreatedEvent());
      await route.fulfill(
        rpcResponse({
          id: "event-e2e",
...
function buildCreatedEvent(): OrganizerEvent {
  return {
    id: "event-e2e",
    name: "E2E整理番号公演",
