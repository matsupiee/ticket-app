...
//   - 整理番号(InventorySlot.entryNumber)は InventoryPool（公演 × 在庫種別）単位の連番
//   - 接頭辞は InventoryCategory.entryNumberPrefix が持つ。同じ公演で「A-1」と「S-1」を見分けるためのもの
...
type FormatEntryNumberInput = {
  // InventoryCategory.entryNumberPrefix。接頭辞なしの在庫種別では null
